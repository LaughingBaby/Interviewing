...
## 一、Go 并发当前能力全景：goroutine、channel、select、锁与 sync 包

> 先掌握并发工具箱：goroutine、channel/select、锁、atomic、WaitGroup、Once。
...
## 二、Go 运行时核心原理：GMP、调度、channel 边界、GC、逃逸与内存复用

> 这一章回答“Go 并发为什么这样工作”：调度、通信、内存和 GC。
...
### P0 补充：Go 内存分配器 mcache、mcentral、mheap 怎么理解？
...
### P0 补充：GC 调优、GOGC、GOMEMLIMIT 与线上指标
...
## 三、线上排障实战：泄漏、CPU、内存、pprof 与优雅停机

> 这一章回答“线上 Go 服务出问题怎么办”。
...
## 四、项目实战表达：把 Go 并发讲成稳定性能力

> 这一章用于项目深挖，把并发控制和排障讲成真实经验。
